...
class modbt_loginHelper
{
	...
	public static function getModules($params) {
		...
		if (count($module_id) > 0) {
			...
			foreach ($modules as $module) {
				...
				if ($module->module != 'mod_vbizz_login') {
					$i++;
	...
	public static function fetchHead($params){
		...
		// load js
		if(file_exists(JPATH_BASE.'/templates/'.$template.'/html/mod_vbizz_login/js/default.js'))
		{	
			if($loadJquery)
			{ 
				$document->addScript(JURI::root(true).'/templates/'.$template.'/html/mod_vbizz_login/js/jquery.min.js');
			}
			$document->addScript(JURI::root(true).'/templates/'.$template.'/html/mod_vbizz_login/js/jquery.simplemodal.js');
			$document->addScript(JURI::root(true).'/templates/'.$template.'/html/mod_vbizz_login/js/default.js');	
		}
		else{
			if($loadJquery)
			{ 
				$document->addScript(JURI::root(true).'/modules/mod_vbizz_login/tmpl/js/jquery.min.js');
			}
			$document->addScript(JURI::root(true).'/modules/mod_vbizz_login/tmpl/js/jquery.simplemodal.js');	
			$document->addScript(JURI::root(true).'/modules/mod_vbizz_login/tmpl/js/default.js');	
		}
		
		// load css
		if(file_exists(JPATH_BASE.'/templates/'.$template.'/html/mod_vbizz_login/css/style2.0.css'))
		{
			$document->addStyleSheet(  JURI::root(true).'/templates/'.$template.'/html/mod_vbizz_login/css/style2.0.css');
		}
		else{
			$document->addStyleSheet(JURI::root(true).'/modules/mod_vbizz_login/tmpl/css/style2.0.css');
		}
...
/**
	...
	public static function register($temp)
	{
		...
		$data['siteurl']	= str_replace('modules/mod_vbizz_login/','',JURI::root());
	...
	public static function createCaptcha(){
		$session = JFactory::getSession();
		$oldImages = glob(JPATH_ROOT . '/modules/mod_vbizz_login/captcha_images/*.png');
		if($oldImages){
		...
		if(!file_exists(JPATH_ROOT . "/modules/mod_vbizz_login/captcha_images")){
			mkdir(JPATH_ROOT . "/modules/mod_vbizz_login/captcha_images");
		}
		
		imagepng($image, JPATH_ROOT . '/modules/mod_vbizz_login/captcha_images/' . $imagePath);
		return JURI::root(). 'modules/mod_vbizz_login/captcha_images/' . $imagePath;
	}
	/**
