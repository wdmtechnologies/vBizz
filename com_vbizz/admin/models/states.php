...
class VbizzModelStates extends JModelLegacy
{
	...
	{
		$query = ' SELECT s.*,c.country_name as country FROM #__vbizz_states as s left join #__vbizz_countries as c on c.id=s.country_id';
		return $query;
	}
	...
	function _buildItemFilter()
	{
		...
		if ($search)
		{
			if(is_numeric($search)) {
				...
				$where[] = 'LOWER(s.state_name) LIKE '.$this->_db->Quote('%'.$search.'%');
			}
		}
	...
	function getItem()
	{
		// Load the data
		if (empty( $this->_data )) {
			$query = 'SELECT * FROM #__vbizz_states WHERE id = '.$this->_id;
	...
	function store()
	{	
		$row = $this->getTable('States', 'VaccountTable');
		...
		if(!$data['id']) {
			$insert->comments = sprintf ( JText::_( 'NEW_NOTES_STATE' ), $data['state_name'], $itemid, 'created', $user->name, $created);
		} else {
			$insert->comments = sprintf ( JText::_( 'NEW_NOTES_STATE' ), $data['state_name'], $itemid, 'edited', $user->name, $created);
		}
	...
	function delete()
	{
		...
		if (count( $cids )) {
			foreach($cids as $cid) {
				...
				$insert->views = "states";
				$insert->type = "data_manipulation";
				$insert->comments = sprintf ( JText::_( 'NEW_NOTES_STATE_DELETE' ), $cid, $user->name, $created);
