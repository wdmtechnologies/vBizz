...
class VbizzModelTax extends JModelLegacy
{
	var $_list;
    var $_data = null;
	var $_total = null;
	...
	function getItem()
	{
		// Load the data
		if (empty( $this->_data )) {
			$query = ' SELECT * FROM #__vbizz_tax WHERE id = '.$this->_id;
	...
	function store()
	{	
		$row = $this->getTable('Tax', 'VaccountTable');
		...
		if($data['id']) {
			$edit_access = $config->tax_acl->get('editaccess');
			if($edit_access) {
				$editaccess = false;
		...
		if(!$data['id']) {
			$add_access = $config->tax_acl->get('addaccess');
			if($add_access) {
				$addaccess = false;
				foreach($groups as $group) {
		...
		// Bind the form fields to the tax table
		if (!$row->bind($data)) {
			$this->setError($row->getError());
			return false;
		}
		// Make sure the tax record is valid
		if (!$row->check()) {
			$this->setError($row->getError());
			return false;
		}
		...
		if(!$data['id']) {
			$insert->comments = sprintf ( JText::_( 'NEW_NOTES_TAX' ), $data['tax_name'], $itemid, 'created', $user->name, $created);
		} else {
			$insert->comments = sprintf ( JText::_( 'NEW_NOTES_TAX' ), $data['tax_name'], $itemid, 'edited', $user->name, $created);
		}
	...
	function delete()
	{
		...
		$delete_access = $config->tax_acl->get('deleteaccess');
		if($delete_access) {
			$deleteaccess = false;
			foreach($groups as $group) {
		...
		if (count( $cids )) {
			foreach($cids as $cid) {
				...
				$insert->views = "tax";
				$insert->type = "data_manipulation";
				$insert->comments = sprintf ( JText::_( 'NEW_NOTES_TAX_DELETE' ), $cid, $user->name, $created);
	...
	function getConfig()
	{
		...
		$registry->loadString($config->tax_acl);
		$config->tax_acl = $registry;
