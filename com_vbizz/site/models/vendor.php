...
class VbizzModelVendor extends JModelList
{
	...
	{
		$query =' SELECT i.* , c.country_name as country, s.state_name as state, (select sum(t.actual_amount-t.discount_amount+t.tax_amount) from #__vbizz_transaction as t where t.types="expense" and t.vid=i.userid) as total_amount FROM #__vbizz_vendor as i left join #__vbizz_countries as c on i.country_id=c.id left join #__vbizz_states as s on i.state_id=s.id ';
		return $query;
	}
	...
	function _buildItemFilter()
	{
		...
		if ($search)
		{
			...
			/* if(is_numeric($search)) {
				$where[] = 'i.userid= '.$this->_db->Quote($search);
			} else {
	...
	function getItem()
	{
		// Load the data
		if (empty( $this->_data )) {
			$query = 'SELECT i.*, u.username as username, v.profile_pic as profile_pic FROM #__vbizz_vendor as i left join #__users as u on i.userid=u.id left join #__vbizz_users as v on i.userid=v.userid WHERE i.userid = '.$this->_db->quote($this->_id).' and i.ownerid='.$this->_db->quote(VaccountHelper::getOwnerId());
		...
		if (!$this->_data) {
			...
			$new_data = $session->get( 'vendData', array() );
	...
	function store()
	{	
		$row = $this->getTable('Vendor', 'VaccountTable');
		$data = JRequest::get( 'post' );
		
		$data['user_role'] = VaccountHelper::getVenderGroup();
		...
		if($data['id']) {
			VaccountHelper::getCheckAuthItem($data['id'], '#__vbizz_vendor','userid');
			$edit_access = $configuration->vendor_acl->get('editaccess');
			if($edit_access) {
				$editaccess = false;
		...
		if(!$data['id']) {
			$add_access = $configuration->vendor_acl->get('addaccess');
			if($add_access) {
				$addaccess = false;
				foreach($groups as $group) {
		...
		if(!$id)	{
			...
			$post['groups'][] = 2;
			$system	= $params->set('new_usertype', VaccountHelper::getVenderGroup());
		...
		$query = 'SELECT count(*) from #__vbizz_vendor where userid='.$u_id;
		$this->_db->setQuery($query);
		...
		if($countCust) {
			if(!$this->_db->updateObject('#__vbizz_vendor', $insert, 'userid'))	{
				$this->setError($this->_db->stderr());
				return false;
			}
		} else {
			if(!$this->_db->insertObject('#__vbizz_vendor', $insert, 'userid'))	{
				$this->setError($this->_db->stderr());
				return false;
			}
		}
		...
		$userss = JFactory::getUser();
		
		
		$insert = new stdClass();
		$insert->id = null;
		$insert->created = $date;
		$insert->created_by = $userss->id;
		$insert->itemid = $u_id;
		$insert->views = $data['view'];
		$insert->type = "data_manipulation";
		
		if(!$data['id']) {
			$insert->comments = sprintf ( JText::_( 'NEW_NOTES_VENDOR' ), $configuration->vendor_view_single, $data['name'], $u_id, 'created', $userss->name, $created);
		} else {
			$insert->comments = sprintf ( JText::_( 'NEW_NOTES_VENDOR' ), $configuration->vendor_view_single, $data['name'], $u_id, 'edited', $userss->name, $created);
		}
	...
	function sendMail(&$user, $password)
	{
		...
		$body = sprintf ( JText::_( 'VENDOR_ACCOUNT_MAIL' ), $user_name, $ownerName, $configuration->vendor_view_single, $siteurl, $sitename, $username, $password);
	...
	function delete()
	{
		...
		$delete_access = $config->vendor_acl->get('deleteaccess');
		if($delete_access) {
			$deleteaccess = false;
			foreach($groups as $group) {
		...
		if (count( $cids )) {
			foreach($cids as $cid) {
				...
				$query = 'DELETE from #__vbizz_vendor WHERE '.$this->_db->quoteName('userid').' = '.$this->_db->quote($cid);
				$this->_db->setQuery( $query );
				if(!$this->_db->query())	{
					$this->setError($this->_db->getErrorMsg());
					return false;
				}
				/* $query = 'DELETE `#__vbizz_transaction`, `#__vbizz_relation` FROM t1 INNER JOIN t2
				WHERE t1.id=t2.transaction_id AND t1.vid='.$this->_db->quote($cid);
				$this->_db->setQuery( $query );
				if(!$this->_db->query())	{
				$this->setError($this->_db->getErrorMsg());
				return false;
				...
				$insert->views = "vendor";
				$insert->type = "data_manipulation";
				$insert->comments = sprintf ( JText::_( 'NEW_NOTES_VENDOR_DELETE' ), $configuration->vendor_view_single, $cid, $user->name, $created);
	...
	{
		$query = 'SELECT country_id from #__vbizz_vendor where userid='.$this->_id;
		$this->_db->setQuery($query);
	...
	function getConfig()
	{
		...
		$registry = new JRegistry;
		$registry->loadString($config->vendor_acl);
		$config->vendor_acl = $registry;
