...
class VbizzModelDeposit extends JModelLegacy
{
	...
	function __construct()
	{
		...
		$context	= 'com_vbizz.deposit.list.';
	...
	{
		$query = 'SELECT d.*, a.account_name as account_name, u.name as userName FROM #__vbizz_deposit as d left join #__vbizz_accounts as a on d.account_id=a.id left join #__users as u on d.created_by=u.id';
		return $query;
	}
	...
	function getItem()
	{
		// Load the data
		if (empty( $this->_data )) {
			$query = ' SELECT * FROM #__vbizz_deposit WHERE id = '.$this->_id;
			$this->_db->setQuery( $query );
			$this->_data = $this->_db->loadObject();
		}
		//if no record, set null
		if (!$this->_data) {
	...
	function store($data)
	{	
		$row = $this->getTable('Deposit', 'VaccountTable');
		...
		//check if user is authorised to add data
		...
		if($data['amount'] == "")	{
			$this->setError(JText::_( 'ENTER_AMOUNT_TO_DEPOSIT' ));
			return false;
		}
		...
		$insert->created = JFactory::getDate('now',$ownertimezone)->__toString();
		$insert->created_by = $this->user->id;
		$insert->ownerid = VaccountHelper::getOwnerId();
		
		if(!$this->_db->insertObject('#__vbizz_deposit', $insert, 'id'))	{
		...
		$query = 'UPDATE #__vbizz_accounts set available_balance='.$new_source_avail.' WHERE id='.$this->_db->quote($data['account_id']);
		$this->_db->setQuery( $query );
		if(!$this->_db->query())	{
		...
		$insert->comments = sprintf ( JText::_( 'NEW_NOTES_MONEY_DEPOSIT' ), $account_name, $this->user->name, $created);
	...
	//get configuration setting detail
	function getConfig()
	{
