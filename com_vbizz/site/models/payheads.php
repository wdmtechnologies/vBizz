...
class VbizzModelPayheads extends JModelLegacy
{
	var $_list;
    var $_data = null;
	var $_total = null;
	...
	function __construct()
	{
		...
		$context	= 'com_vbizz.payheads.list.';
	...
	{
		$query ='SELECT * FROM #__vbizz_payheads ';
		return $query;
	}
//get data listing
	function getItems()
...
//get data detail
	function getItem()
	{
		// Load the data
		if (empty( $this->_data )) {
			$query = ' SELECT * FROM #__vbizz_payheads WHERE id = '.$this->_id.' and `ownerid`='.$this->_db->quote(VaccountHelper::getOwnerId());
		...
		if (!$this->_data) {
			...
			$new_data = $session->get( 'payData', array() );
			//if session not empty set data from session else set null
			if(!empty($new_data)) {
				$this->_data = new stdClass();
				$this->_data->id = $new_data['id'];
				$this->_data->name = $new_data['name'];
				$this->_data->payhead_type = $new_data['payhead_type'];
			} else {
				$this->_data = new stdClass();
				$this->_data->id = null;
				$this->_data->name = null;
				$this->_data->payhead_type = null;
...
//save data into database
	function store()
	{	
		$row = $this->getTable('Payheads', 'VaccountTable');
		...
		// Bind the form fields to the payheads table
		if (!$row->bind($data)) {
			$this->setError($row->getError());
			return false;
		}
		// Make sure the payheads record is valid
		if (!$row->check()) {
			$this->setError($row->getError());
			return false;
		}
		...
		if(!$data['id']) {
			$insert->comments = sprintf ( JText::_( 'NEW_NOTES_PAYHEAD' ), $data['name'], $itemid, 'created', $user->name, $created);
		} else {
			$insert->comments = sprintf ( JText::_( 'NEW_NOTES_PAYHEAD' ), $data['name'], $itemid, 'edited', $user->name, $created);
		}
	...
	function delete()
	{
		...
		if (count( $cids )) {
			foreach($cids as $cid) {
				...
				$insert->created_by = $user->id;
				$insert->itemid = $cid;
				$insert->views = "payheads";
				$insert->type = "data_manipulation";
				$insert->ownerid = VaccountHelper::getOwnerId();
				$insert->comments = sprintf ( JText::_( 'NEW_NOTES_PAYHEAD_DELETE' ), $cid, $user->name, $created);
