...
class VbizzModelEdept extends JModelLegacy
{
	var $_list;
    var $_data = null;
	var $_total = null;
	...
	{
		$query ='SELECT * FROM #__vbizz_employee_dept ';
		return $query;
	}
	...
	//get item detail value
	function getItem()
	{
		// Load the data
		if (empty( $this->_data )) {
			$query = ' SELECT * FROM #__vbizz_employee_dept WHERE id = '.$this->_id.' and `ownerid`='.$this->_db->quote(VaccountHelper::getOwnerId());
			$this->_db->setQuery( $query );
			$this->_data = $this->_db->loadObject();
		}
		if (!$this->_data) {
			$session = JFactory::getSession();
			$new_data = $session->get( 'deptData', array() );
	...
	function store()
	{	
		$row = $this->getTable('Edept', 'VaccountTable');
		...
		if($data['id']) {
			VaccountHelper::getCheckAuthItem($data['id'], '#__vbizz_employee_dept');
		...
		// Bind the form fields to the edept table
		if (!$row->bind($data)) {
			$this->setError($row->getError());
			return false;
		}
		// Make sure the edept record is valid
		if (!$row->check()) {
			$this->setError($row->getError());
			return false;
		}
		...
		if(!$data['id']) {
			$insert->comments = sprintf ( JText::_( 'NEW_NOTES_DEPT' ), $data['name'], $itemid, 'created', $this->user->name, $created);
		} else {
			$insert->comments = sprintf ( JText::_( 'NEW_NOTES_DEPT' ), $data['name'], $itemid, 'edited', $this->user->name, $created);
		}
	...
	function delete()
	{
		...
		if (count( $cids )) {
			foreach($cids as $cid) {
				...
				$insert->views = "edept";
				$insert->type = "data_manipulation";
				$insert->comments = sprintf ( JText::_( 'NEW_NOTES_DEPT_DELETE' ), $cid, $this->user->name, $created);
