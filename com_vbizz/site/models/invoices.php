...
class VbizzModelInvoices extends JModelLegacy
{
	...
	function addcomments(){  
		...
		$insert->date = JFactory::getDate('now',$ownertimezone)->__toString();
		...
		$userdetails = VaccountHelper::UserDetails($insert->created_by);
		$obj = new stdClass();  
		$obj->result="success";
	...
	function _buildItemFilter()
	{
		...
		if($filter_begin)
		{
			$where[]='i.invoice_date >= ' . $this->_db->quote(date("Y-m-d", strtotime($filter_begin)));
		}
		if ($filter_end)
		{
			$where[]='i.invoice_date <= ' . $this->_db->quote(date("Y-m-d", strtotime($filter_end)));
		}
		...
		if(VaccountHelper::checkOwnerGroup()|| VaccountHelper::checkEmployeeGroup()) {
           ...
           $where[] = ' i.customer = '.$this->user->id;
			...
			$where[] = ' (i.created_by IN ('.$this->user->id.') ||  i.customer = '.$this->user->id.')';
		} else if(VaccountHelper::checkClientGroup()	) {
			$where[] = ' i.customer = '.$this->user->id;  
		}  */
		//$where[] = ' ip.income='.$this->user->id;
		
		
		//$where[] = ' i.transaction_type ="income"';
	...
	function getFinalAmount()
	{
		...
		$query='SELECT sum(i.amount-i.discount_amount+i.tax_amount) as final_amount, c.name as customers FROM #__vbizz_invoices as i left join #__vbizz_users as c on i.customer=c.userid';
		$filter = $this->_buildItemFilter();
		$query .= $filter;
		$this->_db->setQuery($query);
	...
	function getItem()
	{
		...
		if (!$this->_data) {
			...
			if(!empty($new_data)) {
				...
				$this->_data->salesman = $new_data['salesman'];
			...
			$this->_data->tax = !empty($this->_data->tax)?json_decode($this->_data->tax):array();
			$this->_data->discount = !empty($this->_data->discount)?json_decode($this->_data->discount):array();
		}
		if(!$this->_data->tax){
			$this->_data->tax = array();
		}
	...
	{	
		...
		if($data['id']) {
			...
			if(!VaccountHelper::WidgetAccess('invoice_acl','editaccess'))
			{
		...
		$creat_income_expense_section = false;
		if(!$data['id']) { 
		$creat_income_expense_section = true;
		$data['form_status'] = 'income';
        $data['to_status'] = 'expense';		
			if(!VaccountHelper::WidgetAccess('invoice_acl','addaccess'))
			{
		...
		if(VaccountHelper::checkVenderGroup() || VaccountHelper::checkClientGroup()){
		 $data['customer'] = $user->id;//VaccountHelper::getOwnerId();	
		}
		$title						= $data['project'];
		$date						= $data['due_date'];
		$transaction_type			= $data['transaction_type'];
		
		if($config->enable_cust==0 && !$data['id']){
		$data['customer']= $config->enable_cust==0?(VaccountHelper::checkVenderGroup()?$user->id:VaccountHelper::getOwnerId()):$data['customer'];
        }   
		if(isset($data['task_id'])) {
		...
		if($config->enable_tax_discount==1)
			...
			for($i=0;$i<count($custom_title);$i++) {
				...
				$customDiscount = isset($custom_discount[$i])?$custom_discount[$i]:array();
				$customTax = isset($custom_tax[$i])?$custom_tax[$i]:array();
				...
				else
				{
					...
					for($d=0;$d<count($customDiscount);$d++)
					{
						...
						//$taxs[] = $tax_detail;
						$n_c_discount = (($customAmount*$discount_detail->discount_value)/100);				
						//$custom_actual_amount = $custom_actual_amount - $n_c_discount;
	...
	//} 
		...
		/* else{
		if($data['tax_inclusive']==1)	
		$data['amount']	=  $data['actual_amount']-$data['tax_amount'];
	    else
		...
		//create income and expense invoice relation
		...
		if(isset($data['task_id']))	
		{
			...
			for($k=0;$k<count($item_id);$k++)
			{
				...
				// changed
				$itemtax		= isset($data['tax'][$itemid])?$data['tax'][$itemid]:array();
				$itemdiscount	= isset($data['discount'][$itemid])?$data['discount'][$itemid]:array();
		  ...
		  if($config->enable_items==1)
		  {
			...
			$t_items = isset($data['item_id'])?$data['item_id']:array();
			for($i=0;$i<count($t_items);$i++) {     
				
				$itemid = $data['item_id'][$i];
				$itmQtys = $item_quantity[$i];
				...
				if(VaccountHelper::checkOwnerGroup()){
					//VaccountHelper::checkOwnerGroup()||VaccountHelper::checkVenderGroup()
				if($countIt) {
					if($countIt!=$itmQtys)
					{
						//if($countIt>$itmQtys){
		...
		if(isset($config->employeecommission) && $config->employeecommission==1&& $row->status==1)
		{
		  if($data['salesman'])
		  {
			$commission = VaccountHelper::employeeCommission($trId, $data['salesman'],'invoice');	  
		  }
...
//delete records
	...
	function getTypes()
	{
		...
		//get listing of all users of an owner
		$cret = VaccountHelper::getOwnerUserListing();
	...
	function getCustomItem()
	{
		...
		if($this->_id) {
			...
			if($projectid) {
				...
				$query = 'select * from #__vbizz_income_task_rel where transaction_id='.$this->_id.' and taskid=0 ORDER BY id asc';
			} else {
	...
	function getInvoice($id, $tax, $discount, $itemid, $item_task)
	{
		...
		if($item_task=="task") {
			...
			$query = 'select * from #__vbizz_invoices where transaction_id = '.$items->transaction_id;
		}
		$this->_db->setQuery( $query );
		...
		for($i=0;$i<count($itemlist);$i++) {
			...
			$item_name = isset($itemlist[$i]->title)?$itemlist[$i]->title:$itemlist[$i]->project;
		...
		$d_html = '<table width="100%">';
		 foreach($t_d_details[0] as $key => $value) { 
					...
					$d_html .= '<tr><td align="left" valign="top" width="60%">'.$d_detail[0].' '.$d_detail[1].'%</td><td align="right" valign="top">'.VaccountHelper::getValueFormat($value).'</td></tr>';
             ...
             $t_html = '<table width="100%">';			 
				...
				$t_html .= '<tr><td align="left" valign="top" width="60%">'.$t_detail[0].' '.$t_detail[1].'%</td><td align="right" valign="top">'. VaccountHelper::getValueFormat($value).'</td></tr>';
				}		
			$t_html .= '</table>';	
	...
	function getInvoice_Multiple($id, $item_task)
	{
		...
		if($item_task=="task") {
			$query = 'select * from #__vbizz_income_task_rel where transaction_id = '.$items->id;
		} else {
			$query = 'select * from #__vbizz_relation where invoice_id = '.$items->id;
		}
		$this->_db->setQuery( $query );
		$itemlist = $this->_db->loadObjectList();
