...
class VbizzModelQuotesexpense extends JModelLegacy
{
	...
	function __construct()
	{
		...
		$context	= 'com_vbizz.quotesexpense.list.';
	...
	{ 
		...
		/* if(VaccountHelper::checkOwnerGroup()) {
			$query = 'SELECT userid from #__vbizz_users where ownerid = '.$this->user->id;
			$this->_db->setQuery($query);
			$u_list = $this->_db->loadColumn();
			
			if (array_search($this->user->id, $u_list) !== false) {
				$key = array_search($this->user->id, $u_list);
			unset($u_list[$key]);
			}
		  $where[] = ' i.created_by in ('.implode(',' , $u_list).')';
		} else {
			...
			$where[] = '( i.created_by in ('.implode(',' , $u_list).') AND i.customer='.$this->user->id.')';
		}
		 */
		
		
		/* if(VaccountHelper::checkOwnerGroup()) {  
			if($filter_status=="owner") {
				$where[] = ' i.created_by = '.$this->user->id;
			} else if($filter_status=="client") {
				$where[] = ' i.created_by in ('.implode(',' , $u_list).')';
			} else {
				$query = 'SELECT ownerid from #__vbizz_users where userid = '.$this->user->id;
				$this->_db->setQuery($query);
	...
	{
		if(!empty($this->_id)){
		$query = ' SELECT * FROM #__vbizz_comment_section WHERE section_name="quotesexpense" AND section_id= '.$this->_id.' order by comment_id';
		$this->_db->setQuery( $query );
		$comment = $this->_db->loadObjectList();
		return $comment;	
		}
		return array();
	}
	...
	{	
		...
		$data['quote_for'] = VaccountHelper::checkOwnerGroup()||VaccountHelper::checkEmployeeGroup()?"expense":"income";
		...
		if($config->enable_items==1)
		{
			...
			for($k=0;$k<count($item_id);$k++)
			{
				...
				$itemamt = $item_amount[$k];//$items_detail->amount;
				$itemtax				= $actTax[$itemid];
				$itemdiscount			= $actDiscount[$itemid];
...
//delete records
	...
	function getMultipleQuotation($id)
	{
		...
		if(strpos($invoice, '{date}')!== false)	{
		  ...
		  if(strpos($invoice, '{comments}')!== false)	{
			...
			if(strpos($multi_invoice, '{description}')!== false)	{
				$multi_invoice = str_replace('{description}', $description, $multi_invoice);
			}
	...
	function approve() {
		...
		$this->_db->setQuery($query);
		$quote_owner = $this->_db->loadResult();
		
		

		//Quote owner is not authorised to approve quotes
		if($quote_owner) {
	...
	function reject()
	{
		...
		//vendor not authorised to reject quotes
		if($quote_owner) {
	...
	function moveInvoice()
	{
		...
		$insert->tax_values = $quotes->tax_values;
		$insert->discount_values = $quotes->discount_values;
