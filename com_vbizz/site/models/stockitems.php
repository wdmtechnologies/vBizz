...
class VbizzModelStockitems extends JModelLegacy
{
	...
	function __construct()
	{
		...
		$context	= 'com_vbizz.stockitems.list.';
	...
	{
		$query = ' SELECT s.* from #__vbizz_stock_items as s';
		return $query;
	}
	//get data listing
	function getItems()
	...
	function _buildItemFilter()
	{
		...
		$group_id = VaccountHelper::checkusergroup($uID);
	...
	function getItem()
	{
		// Load the data
		if (empty( $this->_data )) {
			$query = ' SELECT * FROM #__vbizz_stock_items WHERE id = '.$this->_id;
		...
		if (!$this->_data) {
			...
			if(!empty($new_data)) {
				...
				$this->_data->name = $new_data['title'];
				$this->_data->quantity = $new_data['quantity'];
				$this->_data->user_id = $new_data['user_id'];
				$this->_data->description = $new_data['description'];
				
				$this->_data->update_date = $new_data['update_date'];
	...
	function store()
	{	
		$row = $this->getTable('Stockitems', 'VaccountTable');
