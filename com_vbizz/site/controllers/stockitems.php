...
class VbizzControllerStockitems extends VbizzController
{
	...
	function __construct()
	{
		...
		$config = $this->getModel('stockitems')->getConfig();
	...
	function edit()
	{
		JRequest::setVar( 'view', 'stockitems' );
	...
	function save()
	{
		...
		$model = $this->getModel('stockitems');
		$link = JRoute::_('index.php?option=com_vbizz&view=stockitems');
	...
	function apply()
	{
		...
		if ($model->store()) {
			...
			$session->clear('stockData');
			$msg = JText::_( 'STOCK_SAVED' );
			$link = JRoute::_('index.php?option=com_vbizz&view=stockitems&task=edit&cid[]='.JRequest::getInt('id', 0));
			$this->setRedirect($link, $msg);
		} else {
			$msg = $model->getError();
			jerror::raiseWarning('', $msg);
	...
	function saveNew()
	{
		...
		if ($model->store()) {
			...
			$session->clear('stockData');
			$msg = JText::_( 'STOCK_SAVED' );
			$link = JRoute::_('index.php?option=com_vbizz&view=stockitems&task=edit&cid[]=0');
			$this->setRedirect($link, $msg);
		} else {
			$msg = $model->getError();
			jerror::raiseWarning('', $msg);
	...
	function remove()
	{
		...
		$this->setRedirect( JRoute::_('index.php?option=com_vbizz&view=stockitems'), $msg );
	}
