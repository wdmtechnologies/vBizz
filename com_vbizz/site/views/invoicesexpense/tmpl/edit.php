...
$query = 'SELECT count(*) from #__vbizz_invoices where ownerid='.$ownerid;
$db->setQuery( $query );
...
if($inv_setting==1){
	...
	if($count_inv==0)
	{
		...
		$qry = 'SELECT invoice_number from #__vbizz_invoices where ownerid='.$ownerid.' ORDER BY id DESC LIMIT 1';
		$db->setQuery( $qry );
	...
	if($count_inv==0)
	{
		...
		$qry = 'SELECT invoice_number from #__vbizz_invoices where ownerid='.VaccountHelper::getOwnerId().' ORDER BY id DESC LIMIT 1';  
		$db->setQuery( $qry );
		$last_invoice_number = $db->loadResult();
...
$js .= '
		...
		{              
			...
			jQuery.ajax(
			{
				...
				type: "POST",
				dataType:"json",
				data: {"option":"com_vbizz", "view":"invoicesexpense", "task":"getItemVal", "tmpl":"component", "id":id},
				...
				success: function(data) 
				{
					if(data.result=="success"){
						...
						{
							var amt=accounting.unformat(0, decimal_formating); 
						}else {
...
$jstask = '
		...
		{              
			...
			jQuery.ajax(
			{
				...
				type: "POST",
				dataType:"json",
				data: {"option":"com_vbizz", "view":"invoicesexpense", "task":"getProjectTask", "tmpl":"component", "id":id},
...
function AddMessageSection(){
	...
	jQuery.ajax({
				  ...
				  type: "POST",
				  dataType: "json",
				  data: {"option":"com_vbizz", "view":"invoicesexpense", "task":"addcomments", "tmpl":"component", "section":"invoicesexpense", "section_id":<?php echo (int)$this->invoices->id; ?>, "msg":tinymce.get('comments_message').getContent(), "<?php echo JSession::getFormToken(); ?>":1, 'abase':1},
				  beforeSend: function()	{
...
function view_text () {
	...
	function refreshDashboard() {   
        ...
        var jqxhr = jQuery.ajax({
			...
			dataType: "json",
            url: "index.php",
             data: {"option":"com_vbizz", "view":"invoicesexpense", "task":"UpdateComments", "ptaskid":"<?php echo (int)$this->invoices->id; ?>", "tmpl":"component", "<?php echo JSession::getFormToken(); ?>":1, 'abase':1}
        }).done(function(resp){
            console.log('REFRESH');
...
jQuery(document).on('click','.remove',function() {
	...
	jQuery.ajax(
	{
		...
		type: "POST",
		dataType:"json",
		data: {"option":"com_vbizz", "view":"invoicesexpense", "task":"removeItem", "tmpl":"component", "itemid":itemid, "transaction_id":transaction_id},
...
<form action="<?php echo JRoute::_('index.php?option=com_vbizz&view=invoicesexpense'); ?>" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data">
				...
				<?php if($editaccess) { ?>
				  ...
				  <?php } ?>
					...
					<div class="btn-wrapper"  id="toolbar-pdf">
						<a class="modal btn btn-small" id="modal1" title="Select" href="<?php echo JURI::root().'index.php?option=com_vbizz&view=invoicesexpense&task=print_bill&tmpl=component&cid[]='.$this->invoices->id; ?>" rel="{handler: 'iframe', size: {x: 800, y: 500}}"><span class="fa fa-plus"></span> <?php echo JText::_('PRINT_BILL'); ?></span></a>
...
<?php } ?>
	...
	<tbody>  
        ...
        <tr>
            <th width="200"><label id="project-msg" for="project" class="hasTip" title="<?php if($project_id) {echo JText::_('PROJECTTXT');}else{echo JText::_('INVOICETTXT');} ?>">
                <?php if($project_id) {echo JText::_('PROJECT');}else{echo JText::_('INVOICETTXT');} ?><?php echo JText::_('<span style="color:Red;">'.'*  '.'</span>');?></label>
            </th>
		...
		<tr>
            ...
            <td><?php echo JHTML::_('calendar', $this->invoices->due_date, "due_date" , "due_date", VaccountHelper::DateFormat_javascript($this->config->date_format), ' class="validate-daterangevalidate"'); ?></td>
        </tr>
        <tr>
		...
		<tr>
            <th><label class="hasTip" title="<?php echo sprintf ( JText::_( 'SELTERMDESCTXT' ), JText::_( 'VENDOR' )); ?>">
                <?php echo JText::_( 'VENDOR' ); ?><?php echo JText::_('<span style="color:Red;">'.'*  '.'</span>');?></label>
            </th>
            <td class="sel_customer"><input id="cust" type="text" readonly="" value="<?php if($customer){ echo $customer;} else {echo sprintf ( JText::_( 'SELTERMTXT' ), JText::_( 'VENDOR' ));} ?>">
            <a class="btn btn-primary modal" id="modal" rel="{handler: 'iframe', size: {x: 800, y: 500}}" href="<?php echo JURI::root().'index.php?option=com_vbizz&view=vendor&layout=modal&tmpl=component&for=expense';?>" title="<?php echo sprintf ( JText::_( 'SELTERMTXT' ), JText::_( 'VENDOR' )); ?>">
            <i class="fa fa-user hasTip" title="<?php echo sprintf ( JText::_( 'SELTERMTXT' ), JText::_( 'VENDOR' )); ?>"></i>
            </a>
			</td>
            <input id="customer" type="hidden" value="<?php echo $this->invoices->customer; ?>" name="customer" />
        </tr>
		 <?php	}	?>
		<?php if($this->config->enable_tax_discount==1){ ?>
		<tr>
        ...
        <tr>
            ...
            <div class="multi-item">
                ...
                <div class="item_button">
					<a class="<?php if(!$project_id) { ?>remove<?php } else { ?>remNew<?php } ?> btn" itId="<?php echo $items->id; ?>" href="javascript:void(0);"><span class="loadingbox" style="display:none;"><?php echo '<img src="'.JURI::root().'components/com_vbizz/assets/images/load.gif" />' ?></span><i class="fa fa-remove"></i></a>
				</div>
...
<br>
  ...
  <div class="comment_section_add new">
   ...
   </div><div class="comment_section_add_msg">  
	<?php  
