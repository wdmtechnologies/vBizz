...
jQuery(function() {
	...
	var availableBalance = parseFloat(jQuery('input[name="available_balance"]').val());
	jQuery(document).on('change', 'select[name="account_id"]', function()	{
		...
		jQuery.ajax({
			type: "POST",
			dataType:"JSON",
			data: {'option':'com_vbizz','view':'withdrawl', 'task':'getAvailableBalance', 'tmpl':'component', 'account':account},
	...
	jQuery(document).on('click', '#transfer-money', function()	{
		...
		if(amount == "")	{
			alert("<?php echo JText::_('ENTER_AMOUNT_TO_WITHDRAW'); ?>");
			return false;
		}
		
		if(parseFloat(amount) > parseFloat(availableBalance)) {
			alert("<?php echo JText::_('AMOUNT_VALUE_GREATER_THAN_AVAIL_BALANCE'); ?>");
			return false;
		}
		...
		jQuery.ajax({
			type: "POST",
			dataType:"JSON",
			data: { 'option':'com_vbizz','view':'withdrawl', 'task':'save', 'tmpl':'component', 'account_id':account_id, 'amount':amount },
...
<header class="header">
	<div class="container-title">
		<h1 class="page-title"><?php echo JText::_('WITHDRAW'); ?></h1>
	</div>
</header>
...
<fieldset class="adminform">
	...
	<ul>
		<li><?php	echo JText::_('NEW_WITHDRAW_OVERVIEW');  ?></li>
	</ul>
</fieldset>
</div>
...
<input type="hidden" name="available_balance" id="available_balance" value="<?php echo $this->available_balance; ?>" />
</form>
</div>
