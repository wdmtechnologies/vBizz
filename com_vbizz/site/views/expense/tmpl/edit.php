...
$db = JFactory::getDbo();
if($this->expense->vid) {
	$query = 'SELECT name from #__vbizz_users where userid='.$this->expense->vid;
	$db->setQuery( $query );
		...
		{              
			...
			jQuery.ajax(
			{
				...
				{if(data.result=="success"){
					 ...
					 $js .= ' newhtml +="<div class=item_button><a class=remNew href=javascript:void(0) id=item_setting><i class=\"fa fa-remove\"></i></a></div><div id="+data.itemid+"></div><input type=hidden class=item_id name=item_id[] value="+data.itemid+" /><input type=hidden class=item_title name=item_title[] value="+data.itemtitle+" /></div>"
...
jQuery(document).on('click','.remove',function() {
	...
	var transaction_id = '<?php echo $this->expense->id; ?>';
	var that=this;  
...
<header class="header">
	<div class="container-title">
		<h1 class="page-title"><?php echo isset($this->expense->id)&&$this->expense->id>0?JText::_('EXPENSEEDIT'):JText::_('EXPENSENEW'); ?></h1>
	</div>
</header>

<div class="content_part">
<form action="<?php echo JRoute::_('index.php?option=com_vbizz&view=expense'); ?>" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data">
...
<fieldset class="adminform">
	...
	<ul>
		<li><?php	echo JText::_('NEW_EXPENSE_OVERVIEW');  ?></li>
		<li><?php	echo JText::_('ADD_VENDOR_OVERVIEW');  ?></li>
...
<div class="col100">  
<fieldset class="adminform">
<legend><?php if($this->expense->id) echo JText::_('DETAILS'); else echo JText::_('ADD_NEW');?></legend>
<table class="adminform table table-striped">
<?php if($this->expense->id) { 
$itemTitle = preg_replace('/\s+/', '', $this->expense->title);
$itemName = strtolower($itemTitle);

?>
<div class="v-pdf"><a href='<?php echo JURI::root() . 'components/com_vbizz/pdf/invoice/'.$itemName.$this->expense->id.'invoice'.".pdf" ?>' class="pdf btn"  target="_blank"><label class="hasTip" title="<?php echo JText::_('PDFTXT'); ?>"><i class="fa fa-download"></i> <?php echo JText::_('DOWNLOAD_PDF'); ?></label></a></div>
<?php } ?>
	<tbody>
        <tr>
            ...
            <td><input class="text_area required" type="text" name="title" id="title" value="<?php echo $this->expense->title;?>" /></td>
        </tr>
        ...
        <tr>
            ...
            <td><?php echo JHTML::_('calendar', $this->expense->tdate, "tdate" , "tdate", VaccountHelper::DateFormat_javascript($this->config->date_format),' class="required validate-datevalidate"'); ?></td>
        </tr>
		<?php } ?>
        ...
        <tr>
            ...
            <td><input class="text_area" type="text" name="quantity" readonly="readonly" id="quantity" value="<?php echo VaccountHelper::getNumberFormatValue($this->expense->quantity);?>" /></td>
        </tr>
        ...
        <tr>
            ...
            <td><input class="text_area required" type="text" name="actual_amount" id="actual_amount" readonly="readonly" value="<?php if($this->expense->tax_inclusive==1){echo VaccountHelper::getNumberFormatValue($this->expense->actual_amount+$this->expense->tax_amount);} else {echo VaccountHelper::getNumberFormatValue($this->expense->actual_amount);}?>"/><?php echo ' '.$this->config->currency; ?></td>
        </tr>
        ...
        <tr>
            ...
            <td>
                ...
                <option value="1" <?php if($this->expense->status==1) echo 'selected="selected"'; ?>><?php echo JText::_('PAID'); ?></option>
                <option value="0" <?php if($this->expense->status==0) echo 'selected="selected"'; ?>><?php echo JText::_('UNPAID'); ?></option>
                </select>
            </td>
        </tr>
        ...
        <option value="<?php echo $this->ttype[$i]->id; ?>" <?php if($this->ttype[$i]->id==$this->expense->tid) echo 'selected="selected"'; ?>> <?php echo JText::_($this->ttype[$i]->treename); ?> </option>
        <?php	}	?>
        </select></td>
        </tr>
        ...
        <tr>
            ...
            <td>
                ...
                <option value="<?php echo $this->mode[$i]->id; ?>" <?php if($this->mode[$i]->id==$this->expense->mid) echo 'selected="selected"'; ?>> <?php echo JText::_($this->mode[$i]->title); ?> </option>
                <?php	}	?>
                </select>
            </td>
        </tr>
		...
		<?php if($emp) { ?>
		<tr>
            <th><label class="hasTip" title="<?php echo JText::_('EMPLOYEETXT'); ?>">
            <?php echo JText::_('EMPLOYEE'); ?><?php echo JText::_('<span style="color:Red;">'.'*  '.'</span>');?></label></th>
            <td>
                <select name="employee" id="employee">
                <option value=""><?php echo JText::_('SELECT_EMPLOYEE'); ?></option>
                <?php	for($i=0;$i<count($this->employee);$i++)	{	?>
                <option value="<?php echo $this->employee[$i]->userid; ?>" <?php if($this->employee[$i]->userid==$this->expense->employee) echo 'selected="selected"'; ?>> <?php echo JText::_($this->employee[$i]->name); ?> </option>
                <?php	}	?>
                </select>
            </td>
        </tr>
        ...
        <tr>
            <th><label id="account_id-msg" for="account_id" class="hasTip" title="<?php echo JText::_('SELECTACCTXT'); ?>"><?php echo JText::_('SELECT_ACCOUNT'); ?><?php echo JText::_('<span style="color:Red;">'.'*  '.'</span>');?></label></th>
            <td>
                ...
                <option value="<?php echo $this->account[$i]->id; ?>" <?php if($this->account[$i]->id==$this->expense->account_id) echo 'selected="selected"'; ?>> <?php echo JText::_($this->account[$i]->account_name); ?> </option>
                <?php	}	?>
                </select>
            </td>
        </tr>
		...
		<tr>
			...
			<td>
				...
				<label for="tax_inclusive1" id="tax_inclusive-lbl" class="radio btn"><?php echo JText::_('YS'); ?></label>
				<input type="radio" name="tax_inclusive" id="tax_inclusive1" value="1" <?php if($this->expense->tax_inclusive) echo 'checked="checked"';?> />
				<label for="tax_inclusive0" id="tax_inclusive-lbl" class="radio btn"><?php echo JText::_('NOS'); ?></label>
				<input type="radio" name="tax_inclusive" id="tax_inclusive0" value="0"  <?php if(!$this->expense->tax_inclusive) echo 'checked="checked"';?>/>
				</fieldset>
			</td>
		</tr>
		<?php	}	?>
        <tr>
            ...
            <td><input class="text_area" type="text" name="tranid" id="tranid" value="<?php echo $this->expense->tranid;?>" /></td>
        </tr>
        
        <?php if(!$emp) { ?>
        <?php if($this->config->enable_vendor==1) { ?>
        <tr>
            <th><label id="vid-msg" for="vid" class="hasTip" title="<?php echo sprintf ( JText::_( 'SELTERMDESCTXT' ), $this->config->vendor_view_single); ?>"><?php echo $this->config->vendor_view_single; ?></label></th>
            ...
            <input id="vid" type="hidden" class="required" value="<?php echo $this->expense->vid; ?>" name="vid" />
        </tr>
        <?php	}	?>
        <?php	}	?>
        ...
        <tr>
            ...
            <td><textarea class="text_area" name="comments" id="comment" rows="4" cols="50"><?php echo $this->expense->comments;?></textarea></td>
        </tr>
        ...
        <tr>
			...
			<?php if(isset($this->expense->reciept)) { ?>
                <a target="_blank" href="components/com_vbizz/uploads/reciept/<?php echo $this->expense->reciept;?>"><?php echo $this->expense->reciept;?></a>
			<?php } ?>  
            </td>
        </tr>
        ...
        <tr>
        	...
        	<th colspan="0">
               ...
               <a class="modal" id="modal1" title="Select" href="<?php echo JURI::root().'index.php?option=com_vbizz&view=items&layout=modal&tmpl=component&pro=exp&filter_type='.$this->expense->tid; ?>" rel="{handler: 'iframe', size: {x: 800, y: 500}}">
			...
			foreach($this->multi_item as $key => $items ) {
				$c++;
				...
				<div class="tax-block">
					...
					<span class="sel-tax">
						...
						<?php	for($j=0;$j<count($this->tax);$j++)	{	?>
							<option value="<?php echo $this->tax[$j]->id; ?>" <?php if(in_array($this->tax[$j]->id,$items->tax)) { echo 'selected="selected"';}?>> <?php echo $this->tax[$j]->tax_name; ?> </option>
...
<input type="hidden" name="id" value="<?php echo $this->expense->id; ?>" />
