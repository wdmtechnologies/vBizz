...
$js .= '
		...
		{              
			...
			jQuery.ajax(
			{
				...
				type: "POST",
				dataType:"json",
				data: {"option":"com_vbizz", "view":"quotes", "task":"getItemVal", "tmpl":"component", "id":id},
				...
				success: function(data) 
				{
					if(data.result=="success"){
						...
						if(jQuery("input[name=amount]").val()=="")
						{
							...
							var amt = accounting.unformat(jQuery("input[name=amount]").val(), decimal_formating);
						}
						...
						jQuery("input[name=amount]").val(accounting.formatNumber(new_amt,2,thousand_formating, decimal_formating));
						jQuery("input[name=amount]").attr("readonly","readonly")
						 var i = jQuery("div.multi-item").length;
						...
						if($this->config->enable_tax_discount==1)
						{     
		                 $js .= ' newhtml +="<div class=\"tax-block\"><span class=\"tax_label\">'.JText::_("TAX").': </span><span class=\"sel-tax\"><select class=\"tax\" name=\"tax["+i+"][]\" id=\"tax"+i+"\"  multiple>'.$newcustom_tax.'</select></span></div><div class=\"discount-block\"><span class=\"discount_label\">'.JText::_('DISCOUNT').': </span><span class=\"sel-dis\"><select class=\"discount\" name=\"discount["+i+"][]\" id=\"discount"+i+"\"  multiple>'.  $newcustom_discount.'</select></span></div>";';      
		               }      
		        $js .= ' newhtml +="<div class=item_button><a class=remNew href=javascript:void() id=item_setting><i class=\"fa fa-remove\"></i></a></div><div id="+data.itemid+"></div><input type=hidden class=item_id name=item_id[] value="+data.itemid+" /></div>";
...
jQuery(document).on('change','input.item_quantity',function() {
	...
	jQuery('input[name="amount"]').val(accounting.formatNumber(newItmAmt,2,thousand_formating,decimal_formating));
...
jQuery(document).on('click','.remove',function() {
	...
	jQuery.ajax(
	{
		...
		type: "POST",
		dataType:"json",
		data: {"option":"com_vbizz", "view":"quotes", "task":"removeItem", "tmpl":"component", "itemid":itemid, "quote_id":quote_id},
...
function removeItemSection(removeSectionItems,section)
	...
	var amount = accounting.unformat(jQuery('input[name="amount"]').val(),decimal_formating);
	var itemAmt = accounting.unformat(jQuery(removeSectionItems).parents('.'+section).find('input.'+inputamount).val(),decimal_formating);
	
	var new_amount = (parseFloat(amount)) - (parseFloat(itemAmt));
	jQuery('input[name="amount"]').val(accounting.formatNumber(new_amount,2, thousand_formating, decimal_formating));
...
jQuery(function() {
	...
	jQuery(document).on('change','input.custom_amount_val',function() {
	  ...
	  jQuery('input[name="amount"]').val(accounting.formatNumber(totalamount,2, thousand_formating, decimal_formating));
...
function view_text () {
        ...
        var jqxhr = jQuery.ajax({
			...
			dataType: "json",
            url: "index.php",
             data: {"option":"com_vbizz", "view":"quotes", "task":"UpdateComments", "ptaskid":"<?php echo (int)$this->quotes->id; ?>", "tmpl":"component", "<?php echo JSession::getFormToken(); ?>":1, 'abase':1}
        }).done(function(resp){
            console.log('REFRESH');
    ...
    <div class="span12">
        <div class="btn-toolbar" id="toolbar">
			...
			<?php if($this->quotes->id) { ?>
				...
				<?php if($user->id==$this->quotes->created_by && $this->quotes->approved==0) { ?>
					<div class="btn-wrapper"  id="toolbar-mailing">
...
<form action="<?php echo JRoute::_('index.php?option=com_vbizz&view=quotes'); ?>" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data">
<div class="overview">
...
<?php } ?>
	...
	<tbody>
		...
		<tr>
            ...
            <td><input class="text_area" type="text" name="quantity" readonly="readonly" id="quantity" value="<?php echo VaccountHelper::getNumberFormatValue($this->quotes->quantity);?>"/></td>
        </tr>
        ...
        <tr>
        	<?php if($editaccess) { ?>
        	<th colspan="0">
            	<a class="modal" id="modal1" href="<?php echo JURI::root().'index.php?option=com_vbizz&view=items&layout=modal&tmpl=component';?>" rel="{handler: 'iframe', size: {x: 800, y: 500}}">
            ...
            <div class="multi-item">
                ...
                <div class="amount-block">
                	<span class="item_amount"><?php echo JText::_('AMOUNT'); ?>: </span>
                	<span class="item_amount_value"><input class="item_amount" type="text" autocomplete="off" name="item_amount[]" value="<?php echo $items->amt; ?>"></span>
                </div>
		...
		<tr>
			...
			<div class="custom-line">
				...
				<div class="custom-quantity-block">
					...
					<span class="custom_quantity_value"><input class="custom_quantity_val" type="text" value="<?php echo VaccountHelper::getNumberFormatValue($items->quantity); ?>" name="custom_quantity[]" autocomplete="off"></span>
				</div>
				...
				<div class="custom_button"><a class="remNew btn btn-success" href="javascript:void(0);"><i class="fa fa-remove"></i> </a></div>
