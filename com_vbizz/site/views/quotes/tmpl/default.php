...
<form action="<?php echo JRoute::_('index.php?option=com_vbizz&view=quotes'); ?>" method="post" name="adminForm" id="adminForm">
...
<div class="filter_right filter_block-b">
	<strong><?php echo JText::_( 'VIEW_AS' ); ?>: </strong>
...
<div id="editcell">
    ...
    for ($i=0, $n=count( $this->items ); $i < $n; $i++)	{
		...
		if($userId==$row->created_by) {
			$link 		= JRoute::_( 'index.php?option=com_vbizz&view=quotes&task=edit&cid[]='.$row->id );
		} else {    
			$link 		= JRoute::_( 'index.php?option=com_vbizz&view=quotes&task=details&cid[]='.$row->id );
		}
    ...
    ?>
        <tr class="<?php echo "row$k"; ?>">
			...
			<?php if($userId==$row->created_by) { ?>
				<?php if ($editaccess) : ?>
					<a href="<?php echo $link; ?>"><?php echo $row->title; $income_notification = VaccountHelper::getIncomeNotificationQuote($row->id);if($income_notification>0)echo '<span class="count-note counting_display">'.$income_notification.'</span>';?></a>
				<?php else : ?>
					<?php echo $row->title; ?>
				<?php endif; ?>
